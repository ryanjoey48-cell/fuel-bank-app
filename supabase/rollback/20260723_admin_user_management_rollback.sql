-- Rollback for 20260723_admin_user_management.sql.
-- Review before running. This removes the new account-access model and restores the legacy admin checks.
...
as $$
  select lower(coalesce(auth.jwt() ->> 'email', '')) = 'joeryan09@outlook.com'
...
drop function if exists public.set_account_access_updated_at();
drop function if exists public.is_account_admin();
drop function if exists public.is_account_active();
drop function if exists public.current_account_role();

drop table if exists public.account_access_audit;
